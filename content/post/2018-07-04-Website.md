...
A detailed tutorial for Hugo can be found [here](http://gohugo.io/getting-started/quick-start/). Simply, install {{< hl-text yellow >}}Homebrew{{< /hl-text >}} first, and then:
```
brew install hugo
```
...
cp -a * /Path/to/site/directory/Newsite
...
cp * /Path/to/site/directory/Newsite/archetypes
```
...
Clone the BLOG repository to your local machine.
```
git clone https://github.com/YueYvetteHao/Yue_blog.git
```
Move everything from the {{< hl-text orange >}}Newsite{{< /hl-text >}} to the {{< hl-text orange >}}BLOG{{< /hl-text >}} repo.
```
cd Newsite
cp -a * /Path/to/Yue_blog
```
...
Get the deploy script [here](https://gohugo.io/hosting-and-deployment/hosting-on-github/#put-it-into-a-script) and save it in the {{< hl-text orange >}}BLOG{{< /hl-text >}} repo. Run the shell script to compile the markdown files into html, git add and git commit:
